...
`SKILL.md` references this document at every Stage 3 and Stage 4 confirmation, and at Stage 5 for the center reveal.
...
On surfaces that support in-place artifact updates, emit the artifact once at Stage 3A and update in place at each subsequent stage. Never emit a fresh second copy alongside an existing one.
...
1. **Persistent inline artifact (preferred)** — if the surface supports JSX / HTML / SVG rendering with update-in-place (e.g. Cowork side panel), render ONE artifact at Stage 3A confirmation and update it incrementally through Stage 5. The user watches the same canvas fill in.
...
3. **Markdown only** — if the surface does not support inline rendering at all, skip the visual entirely, deliver a clean markdown summary at Stage 7, and offer Word export.

After the inline visual lands, always offer a follow-up persistent copy in a `~~design tool` (Figma, Canva, Miro) — see `SKILL.md` § Stage 7.
...
**Center Ikigai region:** a small white-fill circle at (550, 470) with r ≈ 75, placed on top of all four overlaps. Empty until Stage 5 confirmation.
...
### Center reveal animation (Stage 5 confirmation)
...
| --- | --- |
| 3A confirmation | First render. Initialize the canvas, populate **Love** circle. Other three circles dashed/empty. |
| 3B confirmation | Populate **Good At** circle (solid stroke, items in crescent) |
| 3C confirmation | Populate **Paid For** circle |
| 3D confirmation | Populate **World Needs** circle |
| 4A confirmation | Reveal **Passion** label + synthesis in its lens |
| 4B confirmation | Reveal **Profession** label + synthesis |
| 4C confirmation | Reveal **Mission** label + synthesis |
| 4D confirmation | Reveal **Vocation** label + synthesis |
| 5 confirmation | Fade in center circle, the inner Ikigai statement, and the callout block (400ms) |

Stages 6 (Business Direction Exploration) and 7 (Final Summary) do **not** modify the artifact — the diagram is complete after Stage 5.
...
### Center Ikigai placement (post-Stage-5)
...
<div class="ikigai-canvas">
  ...
  <svg class="ikigai-svg" viewBox="0 0 1100 900" preserveAspectRatio="xMidYMid meet">
    ...
    <!-- CENTER IKIGAI — add 'revealed' class to circle and text on Stage 5 confirmation -->
    <circle cx="550" cy="470" r="75" class="center-circle" />
    <text x="550" y="475" text-anchor="middle" class="center-text">IKIGAI</text>
  </svg>

  <!-- CENTER CALLOUT — add 'revealed' class on Stage 5 confirmation -->
...
- **Intersection synthesis**: shorten the user's confirmed synthesis statement to fit two lines max in the lens. If it can't, abbreviate — the full version lives in the textual recap at Stage 7.
- **Center reveal (Stage 5)**: add `revealed` class to `.center-circle`, `.center-text`, and `.ikigai-callout`. CSS handles the 400ms fade.
...
1. The artifact appears after **Stage 3A confirmation** (not at Stage 7).
...
5. The center Ikigai statement, once revealed at Stage 5, is the visual focal point (callout below the diagram, plus center anchor).
